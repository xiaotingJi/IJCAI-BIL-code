result<-function(pn,m){
  ...
  x1<-array(runif(2*pn,0,5),c(pn,2))#协变量
  thet<-c(1.5,1,8) #要估计的参数
  y1<-cbind(1,x1)%*%thet+rnorm(pn)#模型
  ...
  result2<-btsgd(x1,y1,error=1,maxiter=m,step=0.001)
  r=result2[[1]]
  return(r)
}

#################################################################################
nn<-seq(100,300,by=10)
...
plot(nn,Power[,1],type="l",ylab="power",xlab="",col="blue",ylim = c(0,1))
plot(nn,PVALUE[,1],type="l",ylab="Pvalue",xlab="",col="red",ylim = c(0,0.2))
y<-rep(0.05,21)
lines(nn,y,ylab="Pvalue",col="black",xlab="")
